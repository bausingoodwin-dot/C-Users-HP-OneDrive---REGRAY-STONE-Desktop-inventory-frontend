window.addEventListener("DOMContentLoaded", () => {
  ...
  function handleStock(isIn) {
    const index = parseInt(productSelect.value);
    const qty = parseInt(document.getElementById("stockQty").value);
    if(isNaN(index) || !qty) return;
    if(!isIn && products[index].quantity < qty){ alert("Not enough stock"); return; }
    products[index].quantity += isIn ? qty : -qty;
    history.push({date:new Date().toLocaleString(), product:products[index].name, type:isIn?"IN":"OUT", qty});
    saveData();
    renderInventory();
  ...
  updateImageBtn.onclick = function() {
    ...
    if(isNaN(index)) { alert("Please select a product"); return; }
